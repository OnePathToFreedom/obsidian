...
В чём была основная проблема, которую решает сабнеттинг? / What's the core problem subnetting solves?::RU: **Subnetting (сабнеттинг)** решает проблему того, что **слишком много** устройств нельзя разумно подключить к одному роутеру. Например, один Class A network содержит 16 777 216 адресов — это далеко не масштаб для одного роутера. Проблема не в нехватке адресов, а именно в перегрузке. EN: **Subnetting** solves the problem that **too many** devices can't reasonably connect to a single router. E.g. one Class A network has 16,777,216 addresses — far too many for one router to handle. The problem isn't an address shortage, it's overload.
Чем gateway router отличается от core router? / How does a gateway router differ from a core router?::RU: **Gateway router** служит точкой входа и выхода (ingress/egress) для конкретной сети — именно он доставляет пакет внутри сети нужному устройству по Host ID. **Core-роутеры** интернета, в отличие от него, общаются **только с другими core-роутерами** и направляют трафик по Network ID к нужному gateway router. EN: A **gateway router** serves as the entry/exit (ingress/egress) point for a specific network, delivering packets inside it by Host ID. Internet **core routers**, in contrast, only talk to **other core routers**, routing traffic by Network ID to the right gateway router.
У каждой подсети — общий или свой собственный gateway router? / Does each subnet share a gateway router, or does it have its own?::RU: У **каждой** подсети — свой **собственный** gateway router, служащий точкой входа/выхода именно для этой подсети (не общий на всю исходную большую сеть). Именно это и решает проблему перегрузки одного роутера. EN: **Each** subnet gets its **own** gateway router serving as its ingress/egress point (not a shared one for the whole original large network). This is exactly what solves the single-router overload problem.
